...
-- neo-tree reports { source, destination }; moves change include paths just like renames
function M.neo_tree_event_handlers()
	local function handler(args)
		M.on_node_renamed({ old_name = args.source, new_name = args.destination })
	end

	return {
		{ event = "file_renamed", handler = handler },
		{ event = "file_moved", handler = handler },
	}
end
